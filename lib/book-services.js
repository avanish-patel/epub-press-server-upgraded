...
class BookServices {
    ...
    */
    ...
    static publish(book) {
        return BookServices.setStatus(book, STATUS_TYPES.PUBLISHING)
            ...
            // Not needed, since amazon stop supporting mobi types
    ...
    */
    ...
    static convertSectionContent(section) {
        ...
        return new Promise((resolve, reject) => {
            try {
                // First try htmltidy2 (original approach)
                tidy(
                    section.content,
                    {
                        outputXhtml: true,
                    ...
                    (err, xhtml) => {
                        if (err) {
                            // Check if it's the "unsupported execution platform" error
                            if (err.message && err.message.includes('unsupported execution platform')) {
                                // Fall back to system tidy
                                log.info('Falling back to system tidy due to platform error');
                                BookServices.convertSectionContentWithSystemTidy(section, resolve, reject);
                            } else {
                ...
                // Handle synchronous errors (like the platform error thrown by chooseExec)
                if (error.message && error.message.includes('unsupported execution platform')) {
                    // Fall back to system tidy
                    log.info('Caught platform error, falling back to system tidy');
                    BookServices.convertSectionContentWithSystemTidy(section, resolve, reject);
                } else {
                    log.exception('BookServices.convertSectionContent')(error);
                    reject(error);
                }
            }
        });
    ...
    static convertSectionContentWithSystemTidy(section, resolve, reject) {
        const updatedSection = section;
        const tidyArgs = [
            '-asxhtml',  // output XHTML
            '-f', '/dev/null',  // suppress error output to file
            '--show-body-only', 'yes',  // only output body content
            '--drop-empty-elements', 'yes',  // drop empty elements
            '--doctype', 'omit'  // omit doctype
        ];

        const child = spawn('tidy', tidyArgs);
        ...
        child.on('close', (code) => {
            if (code !== 0 && errors) {
                log.exception('BookServices.convertSectionContent')(new Error(errors));
                reject(new Error(errors));
            } else {
                // Tidy outputs the entire HTML document, we only want the body content
                const bodyMatch = xhtml.match(/<body[^>]*>([\s\S]*)<\/body>/i);
                const content = bodyMatch ? bodyMatch[1] : xhtml;
                updatedSection.xhtml = content || section.content; // fallback to original if no body content
                resolve(section);
            }
        });

        // Send the content to tidy
        child.stdin.write(section.content);
        child.stdin.end();
    }
